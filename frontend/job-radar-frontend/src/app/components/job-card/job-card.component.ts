...
@Component({
  ...
  template: `
    <article class="cyber-card animate-slide-up group relative overflow-hidden">
      ...
      <!-- Banner DEMO quando Mock -->
      @if (isDemo) {
        <div class="mb-3 -mx-4 -mt-4 px-4 py-2 text-xs font-mono flex items-center gap-2"
             style="background:rgba(251,191,36,0.08);border-bottom:1px solid rgba(251,191,36,0.2);color:#fbbf24">
          ⚠️ Dados de demonstração — configure um provedor real para ver vagas reais
        </div>
      }
      ...
      <div class="flex items-center justify-between pt-3 border-t border-cyber-border/50">
        ...
        <!-- Botão: mock abre busca real no LinkedIn, real abre a vaga diretamente -->
        <a [href]="result.url"
           target="_blank"
           ...
           [title]="isDemo ? 'Abre busca real no LinkedIn (vaga é demonstração)' : 'Abre a vaga original'">
          @if (isDemo) {
...
export class JobCardComponent {
  ...
  get isDemo(): boolean {
    return this.result.title.startsWith('[DEMO]') ||
           this.result.source === 'Mock' ||
           this.result.url.includes('jobs/search');
  }
